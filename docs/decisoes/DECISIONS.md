...
## D-22: Reintroduzir XGBoost como 4º modelo na versão simplificada (parcialmente reverte D-21)

- **Contexto:** D-21 removeu o XGBoost da Etapa 3 porque ele não consta na lista de algoritmos permitida pela disciplina. Pedido explícito nesta iteração: testar o XGBoost mesmo assim e incorporá-lo como modelo oficial, para verificar se um ensemble por *boosting* rompe o teto de ROC-AUC ~0,61–0,62 que os 3 modelos da lista (linear, árvore única, bagging) atingiram juntos.
- **Evidência (`reports/ml/metrics.csv`, gerado por `python -m src.ml.modelagem`):** no teste, XGBoost lidera as 5 métricas — Accuracy 0,6001, Precision 0,3607, Recall 0,5599, F1 0,4388, ROC-AUC 0,6296 — contra Regressão Logística (F1 0,4349, ROC-AUC 0,6198), Árvore de Decisão (F1 0,4265, ROC-AUC 0,6118) e Random Forest (F1 0,4301, ROC-AUC 0,6211). A vantagem é real e consistente em toda métrica, mas pequena em termos absolutos (+0,004 Accuracy, +0,010 ROC-AUC sobre a Regressão Logística) — mesma ordem de grandeza do ganho observado na versão complexa anterior antes de D-21 (D-20: F1 0,4439 com tuning, sem tuning aqui F1 0,4388). O gap validação→teste do XGBoost (F1 0,453→0,439) é o maior dos quatro, mas ainda pequeno em unidades absolutas — sem sinal de overfitting grave.
- **Decisão:** Adicionar XGBoost como 4º modelo em `src/ml/modelagem.py` (`scale_pos_weight` = negativos/positivos do treino, equivalente ao `class_weight="balanced"` dos demais; `max_depth=6` menor que os outros porque boosting soma muitas árvores rasas em vez de poucas profundas), mantendo os 3 modelos da lista da disciplina como estão. Sem busca de hiperparâmetros (mesma regra dos demais modelos desta etapa, e D-20 já havia mostrado ganho de tuning isoladamente desprezível). `docs/entregas/etapa3-modelagem.md`, `src/ml/README.md` e a página de Modelagem do Streamlit foram atualizados para refletir 4 modelos em vez de 3.
- **Alternativas consideradas:** (a) manter XGBoost fora e só documentar os números antigos de D-20 — rejeitada, pedido explícito era testar de novo sobre a base/split atuais (D-21 já havia mudado o split para 70/15/15 desde então); (b) substituir os 3 modelos da lista pelo XGBoost sozinho — rejeitada, a lista de algoritmos da disciplina continua sendo requisito da entrega, XGBoost entra como adicional, não como substituto.
- **Justificativa:** O pedido explícito desta iteração é reintroduzir o XGBoost como modelo oficial; mantê-lo como 4º modelo (em vez de substituir os 3 da lista) preserva o requisito da disciplina enquanto responde à pergunta de fundo (um ensemble por boosting supera bagging/linear/árvore única neste problema?) de forma direta e comparável.
- **Impacto:** `src/ml/modelagem.py::build_models()` ganha `XGBClassifier`; `requirements.txt` recupera `xgboost` e `joblib` (D-21 os havia removido); `train_and_evaluate()` agora salva o pipeline do melhor modelo (por F1) em `reports/ml/model.joblib`, consumido pela nova página 🔮 Teste a Predição (ver D-23).

## D-23: Adicionar seção "Resultados" ao dashboard (resumo de EDA, Modelagem e simulador interativo), mantendo as páginas de aprofundamento intactas

- **Contexto:** O dashboard tinha 13 páginas de análise técnica, todas necessárias para o registro/aprofundamento, mas sem um ponto de entrada único que resumisse "o que a EDA e a Modelagem encontraram" para quem não vai navegar por todas.
- **Decisão:** Criar um grupo de navegação "Resultados" com 3 páginas novas — 📋 Resultados EDA (resumo executivo dos achados mais fortes da Etapa 2, com números recalculados ao vivo), 🏆 Resultados Modelagem (a página 🤖 Modelagem existente, apenas reagrupada) e 🔮 Teste a Predição (simulador interativo que carrega `reports/ml/model.joblib` e aplica o pipeline treinado a um cenário hipotético descrito na hora pelo usuário) — posicionado logo após "Visão Geral" na barra lateral. Nenhuma página de aprofundamento (Qualidade e Estrutura, Análise Exploratória, Preparação para ML) foi removida ou teve conteúdo reduzido; 💡 Insights e Hipóteses foi realocada para "Análise Exploratória" por ser também uma página de aprofundamento (síntese com evidência estatística completa), não um resumo executivo.
- **Alternativas consideradas:** (a) uma única página com `st.tabs` para os 3 itens — rejeitada, o padrão de navegação do app já é por página (`st.navigation`/`st.Page`), misturar os dois padrões confundiria a navegação; (b) substituir as páginas de aprofundamento pelo resumo — rejeitada explicitamente pelo pedido ("manter as outras páginas para análise mais profunda e registrada").
- **Justificativa:** Resumo executivo e registro de aprofundamento são objetivos diferentes que pedem páginas diferentes — a página de resumo prioriza 3–4 achados com 1 gráfico cada; a página de aprofundamento prioriza checagem, intervalo de confiança e todas as hipóteses. Nenhuma reduz a outra.
- **Impacto:** `app/app.py` (dicionário de navegação); `app/pages/13_📋_Resultados_EDA.py` e `app/pages/14_🔮_Teste_a_Predicao.py` (novos); `src/ml/modelagem.py` (persiste `model.joblib`, ver D-22).

## D-24: Simplificar "Teste a Predição" para 3 inputs (UF, hora, dia da semana); enriquecer "Resultados EDA" com geografia e feriados

- **Contexto:** Pedido explícito de simplificar o simulador de predição para pedir só 3 informações do usuário (as mais fáceis de descrever de um cenário, sem exigir conhecimento da base), e de trazer para 📋 Resultados EDA a distribuição por UF com um mapa do Brasil e uma seção sobre feriados.
- **Decisão (predição):** O formulário de 🔮 Teste a Predição pede só `uf`, `dia_semana` e `hora`. As outras 26 features do pipeline são preenchidas automaticamente: `latitude`/`longitude`/`km` = média/mediana **real da UF escolhida** (`AVG`/`MEDIAN` ao vivo em `gold.dataset_ml`, não um centróide fixo); `tipo_pista`/`sentido_via`/`uso_solo`/`mes` = moda global da base; `fase_dia` = aproximação simples por faixa de hora (5-6h Amanhecer, 6-18h Pleno dia, 18-19h Anoitecer, resto Plena Noite) — explicitamente rotulada como aproximação, já que a `fase_dia` real vem do boletim de ocorrência e depende de nascer/pôr do sol por data/latitude; `br` = não informado; traçado = reta (71,6% da base), sem elementos especiais. Tudo isso é exibido a quem usa a página (expander "Ver os valores usados"), nunca escondido.
- **Alternativas consideradas (predição):** (a) retreinar um modelo só com essas 3 features — rejeitada, o pedido é simplificar a *interface*, não descartar o restante do trabalho de feature engineering da Etapa 3; um modelo com só 3 features teria desempenho pior e não representaria a Etapa 3 de verdade; (b) pedir as 29 features no formulário (versão anterior) — trocada pela versão de 3 campos por pedido explícito, mas mantendo o pipeline completo por trás.
- **Decisão (EDA):** 📋 Resultados EDA ganha duas seções novas: "Geografia" (bar chart de acidentes por UF colorido por % grave + mapa do Brasil via `plotly.express.scatter_geo`, bolhas posicionadas na latitude/longitude **média real** de cada UF — sem depender de um GeoJSON externo nem de coordenadas fixas digitadas à mão) e "Feriados" (comparação de acidentes/dia entre Dia comum/Véspera de feriado/Feriado/Pós-feriado, tabela de % grave por tipo de dia, e ranking dos feriados com mais acidentes por `nome_feriado`) — ambas recalculadas ao vivo do banco, mesmo padrão das demais seções da página.
- **Justificativa:** Consistência com o resto do dashboard: nenhum número é digitado à mão, toda simplificação de UX é acompanhada da limitação explícita na tela (regra já seguida em D-21/D-23).
- **Impacto:** `app/pages/14_🔮_Teste_a_Predicao.py` reescrita; `app/pages/13_📋_Resultados_EDA.py` ganha as seções 3 (Geografia) e 4 (Feriados), renumerando a antiga seção 3 para 5; `app/pages/11_🤖_Modelagem.py` ganha os expanders "Por que os 4 resultados ficam tão próximos?" e "Próximos passos".

## D-25: Remover a página "Teste a Predição" (reverte a parte de persistência de modelo de D-22 e as partes de D-23/D-24 relativas ao simulador)

- **Contexto:** Pedido explícito de remover o simulador interativo de predição do dashboard.
- **Decisão:** `app/pages/14_🔮_Teste_a_Predicao.py` foi excluída e removida do dicionário de navegação em `app/app.py` (grupo "Resultados" fica com 2 páginas: 📋 Resultados EDA e 🏆 Resultados Modelagem). Como nada mais no projeto usava o pipeline serializado, `src/ml/modelagem.py` deixou de salvar `reports/ml/model.joblib` (o artefato existia só para essa página), e `joblib` saiu de `requirements.txt`. A escolha do XGBoost como 4º modelo oficial (D-22) **não** é revertida — só a parte de persistência do pipeline treinado.
- **Alternativas consideradas:** manter `model.joblib` mesmo sem a página que o consumia — rejeitada, artefato órfão sem consumidor é exatamente o tipo de complexidade que D-21 já havia removido uma vez (simulador de risco em produção).
- **Justificativa:** Remover uma feature por completo inclui remover a infraestrutura que existia só para ela — deixar `model.joblib` sendo gerado sem nenhuma página que o leia seria um artefato morto no relatório.
- **Impacto:** `app/pages/14_🔮_Teste_a_Predicao.py` (excluída); `app/app.py` (nav); `src/ml/modelagem.py` (remove import de `joblib` e o bloco de persistência); `requirements.txt` (remove `joblib`); `docs/entregas/etapa3-modelagem.md`, `src/ml/README.md`, `README.md` atualizados para não citar mais a página nem `model.joblib`.
