"""
Orquestra a Análise Exploratória de Dados (EDA) completa do dataset de
acidentes da PRF e grava artefatos reproduzíveis em `reports/eda/`.

Roda sobre a camada **Bronze** (tabelas `bronze.*` de `data/prf.duckdb`), não sobre a Silver:
foi a base completa, com a janela final não consolidada, que permitiu
descobrir o problema de consolidação em primeiro lugar (D-13). Os artefatos
gerados aqui (37 blocos + 26 CSVs) são a evidência histórica que embasa o
corte aplicado na Silver (`src/silver/acidentes.py`).

Uso:
    python src/eda/run.py

Saídas:
    reports/eda/eda_results.json   -> todos os números citados em docs/analises/EDA.md,
                                       docs/analises/ANALYSIS_LOG.md e docs/decisoes/DECISIONS.md
    reports/eda/tables/*.csv       -> tabelas de apoio (contingência, séries)

Nenhum número em docs/analises/EDA.md, docs/analises/ANALYSIS_LOG.md ou docs/decisoes/DECISIONS.md deve
...
# Permite rodar tanto como módulo (`python -m src.eda.run`) quanto como
# script direto (`python src/eda/run.py`) — nos dois casos, a raiz do
# projeto precisa estar no sys.path para o import absoluto abaixo funcionar.
