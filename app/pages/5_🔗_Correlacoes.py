...
with tab1:
    ...
    st.error(
        "🚨 **Lembrete de Leakage:** `tipo_acidente` e `causa_acidente` vão aparecer no topo deste ranking, "
        "mas são apuradas **depois** do acidente (pós-evento) — não podem virar feature preditiva direta "
        "(ver 🧠 Features para ML)."
    )
    if where != "1=1":
        st.info("Os filtros da sidebar afetam esta tabela — desative-os para reproduzir os valores de docs/analises/EDA.md.")
    rows = []
    ...
    with right:
        st.dataframe(assoc_df, width="stretch", height=400)
