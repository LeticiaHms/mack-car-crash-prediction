...
with tab_num:
    ...
    if df.empty:
        ...
        # Linha 1: estatísticas básicas, sempre visíveis.
        ...
        # Linha 2: forma da distribuição, sob demanda (menos usada no dia a dia).
        with st.expander("📐 Assimetria e curtose (forma da distribuição)"):
            e1, e2 = st.columns(2)
            e1.metric("Assimetria (skew)", f"{stats.skew(s):.2f}")
            e2.metric("Curtose", f"{stats.kurtosis(s):.2f}")

        log_y = st.toggle("Escala logarítmica no eixo Y", value=False, key="dist_num_log")

        left, right = st.columns(2)
        with left:
            fig = px.histogram(df, x="v", nbins=40, title=f"Distribuição de {col}", log_y=log_y)
            st.plotly_chart(fig, width="stretch")
        with right:
            fig2 = px.box(df, x="gravidade_4", y="v", title=f"{col} por gravidade", log_y=log_y,
        ...
        if col in ("mortos", "feridos_graves", "feridos_leves"):
            st.info(
                ...
                "Veja a página 🚨 Anomalias para a detecção correta via z-score (ver docs/decisoes/DECISIONS.md D-06)."
            )
