...
(quase tudo dá p < 0,001). Quem decide relevância é o tamanho de efeito ([D-08](../docs/DECISIONS.md)).
"""
import numpy as np
import pandas as pd
...
st.set_page_config(page_title="Validação Estatística", page_icon="🧪", layout="wide")
...
with tab_prop:
    ...
    if len(cats) < 2:
        ...
        c1, c2 = st.columns(2)
        cat_a = c1.selectbox("Categoria A", cats["c"].tolist(), index=0)
        cat_b = c2.selectbox("Categoria B", cats["c"].tolist(), index=1)
        ...
        veredito = (
            "diferença real (o IC não cruza zero)"
            if res["diff_ci_pp"][0] * res["diff_ci_pp"][1] > 0
            else "**não distinguível de zero** com estes dados"
        )
        st.markdown(
    ...
    if data["g"].nunique() < 2:
        ...
        if num_col in ("pessoas", "ilesos", "feridos_leves"):
            st.warning(
                ...
                "caracteriza o fenômeno, não valida uma feature preditiva ([D-07](../docs/DECISIONS.md))."
            )
...
with tab_poder:
    ...
    st.info(
        ...
        3. o p-valor entra apenas como checagem de que o achado não é ruído — nunca como medida de
           importância ([D-08](../docs/DECISIONS.md)).
        """.replace(",", ".")
    )
