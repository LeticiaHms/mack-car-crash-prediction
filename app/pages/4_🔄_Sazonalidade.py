...
tab_mensal, tab_semana_hora, tab_feriados = st.tabs(
    ["Análise Mensal", "Dia da Semana & Horário", "Impacto dos Feriados"]
)
...
with tab_mensal:
    ...
    if my.empty:
        ...
        if len(complete_years) >= 2:
            ...
            with st.expander("Detalhes da recorrência (anos comparados e matriz completa)"):
                st.caption(
                    f"Anos completos comparados: {complete_years}. Valor próximo de 1 = o mesmo mês tende "
                    "a ranquear igual (alto/baixo) em anos diferentes → evidência de sazonalidade real de volume."
                )
...
with tab_semana_hora:
    ...
    st.info("Pico de volume ~18h, mas pico de gravidade relativa à noite (19h–23h) — achado A-11.")

# ---------------------------------------------------------------------------
with tab_feriados:
    st.caption(
        "Cruzamento com o calendário de feriados nacionais (63 feriados, 2022–2026). Cada dia da série é "
        "classificado em Feriado / Véspera / Pós-feriado / Dia comum — a hipótese de senso comum é que "
        "feriado concentra acidentes graves; abaixo ela é testada, não assumida."
    )
    ...
    if diario.empty or diario["tipo_dia"].nunique() < 2:
        ...
        if not vesp.empty:
            ...
            with st.expander("📋 Conclusão: o efeito de feriado é de véspera, e é de volume — não de gravidade"):
                st.markdown(
                    f"""
        ...
        with st.expander("⚠️ Ressalva metodológica — feriado não é sorteado no calendário"):
            st.markdown(
                "Feriados caem desproporcionalmente em certos dias da semana, e o dia da semana já tem "
                "efeito próprio sobre volume e gravidade. A comparação acima **não** controla isso — para "
                "separar os dois efeitos, use a página ✂️ Segmentação com `tipo_dia` como segmento e "
                "`dia_semana` como confundidor. É exatamente o caso em que a comparação bruta pode enganar."
            )
