...
específico: visão geral → **resultados** (resumo executivo de EDA e
Modelagem) → qualidade/estrutura da base → exploração → preparação para
a etapa de ML. O grupo "Resultados" existe
para quem quer o essencial sem navegar pelas páginas de aprofundamento;
todas as páginas de análise profunda/registrada (Qualidade e Estrutura,
Análise Exploratória, Preparação para ML) continuam intactas e são as
únicas fontes de verdade para o detalhamento estatístico completo. A
ordem dentro de cada grupo é a ordem de navegação; os arquivos em
`pages/` continuam nomeados como antes só por histórico/compatibilidade
— quem decide agrupamento e ordem agora é este dicionário, não mais o
prefixo numérico do nome do arquivo.
"""
...
pg = st.navigation(
    {
        ...
        "Resultados": [
            st.Page("pages/13_📋_Resultados_EDA.py", title="Resultados EDA", icon="📋"),
            st.Page("pages/11_🤖_Modelagem.py", title="Resultados Modelagem", icon="🏆"),
        ],
