...
st.set_page_config(page_title="Segmentação", page_icon="✂️", layout="wide")
...
st.divider()
st.subheader("2. Diferença observada (com a incerteza que ela merece)")
...
st.markdown(
    f"""
    **Como ler:** o intervalo de confiança da diferença vai de **{ic_lo:+.2f}** a **{ic_hi:+.2f} p.p.**
    (p = {test['p_value']:.2e}). {"Ele não cruza o zero, então a diferença não é ruído amostral"
    if sig and ic_lo * ic_hi > 0 else "Ele **cruza o zero**, então os dados são compatíveis com nenhuma diferença real"}.
    Mas significância não é relevância: o h de Cohen classifica este efeito como **{test['efeito']}**.
    Com n desta ordem, quase toda diferença fica "significante" — é o tamanho do efeito que decide se ela importa
    ([D-08](../docs/DECISIONS.md)).
    """
)

st.divider()
st.subheader("3. A diferença sobrevive ao controle de composição?")
st.caption(
...
st.markdown("**Decomposição por estrato** — onde exatamente A e B diferem:")
det = counts.copy()
...
plot_df = det.melt(id_vars="estrato", value_vars=["taxa_A_%", "taxa_B_%"],
                   var_name="segmento", value_name="pct_grave")
fig2 = px.bar(plot_df, x="estrato", y="pct_grave", color="segmento", barmode="group",
              color_discrete_map={"taxa_A_%": "#3b6ea5", "taxa_B_%": "#d98032"},
              title=f"% grave/fatal por {strata}, dentro de cada segmento",
              labels={"pct_grave": "% grave ou fatal"})
st.plotly_chart(fig2, width="stretch")
st.dataframe(det[["estrato", "n_a", "taxa_A_%", "n_b", "taxa_B_%", "peso_%"]], width="stretch")
st.caption(
    "Estratos com `n` pequeno em um dos lados têm taxa instável — leia junto com a coluna de `n`, "
    "nunca só a barra. A página 🧪 Validação Estatística calcula o IC de cada estrato."
)

st.divider()
st.subheader("4. Perfil comparado dos segmentos")
